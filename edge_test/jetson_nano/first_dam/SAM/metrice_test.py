...
output_folder = "/home/uwrl/Model/segmented_image/SAM/first_dam"
...
# ====== Load SAM Model ======
model_type = "vit_b"
checkpoint_path = "/home/uwrl/Model/SAM5122weights_ViTB.pth"
...
def tegrastats_logger(csv_path, stop_event):
    with open(csv_path, 'w', newline='') as f:
        ...
        for line in process.stdout:
            ...
            emc = re.search(r"EMC_FREQ (\d+%)", line)
            p_in = re.search(r"POM_5V_IN (\d+)/", line)
            p_cpu = re.search(r"POM_5V_CPU (\d+)/", line)
            p_gpu = re.search(r"POM_5V_GPU (\d+)/", line)
            t_cpu = re.search(r"CPU@([\d.]+)C", line)
            t_gpu = re.search(r"GPU@([\d.]+)C", line)
            ...
            writer.writerow({
                ...
                "Power_Total_mW": int(p_in.group(1)) if p_in else None,
                "Power_CPU_mW": int(p_cpu.group(1)) if p_cpu else None,
                "Power_GPU_mW": int(p_gpu.group(1)) if p_gpu else None,
                "Temp_CPU_C": float(t_cpu.group(1)) if t_cpu else None,
                "Temp_GPU_C": float(t_gpu.group(1)) if t_gpu else None,
            })
            f.flush()
